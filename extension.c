/* this file automatically generated by ./makeext.pl extension redirect.txt */
...
int redirecting_sys_stat(const char *file_name, struct stat *buf)
{
	char local0[REDIR_BUFSIZE];
	struct stat local1;
	if(strncpy_from_user(local0, file_name, REDIR_BUFSIZE)<0) return -EFAULT;
	if(redirect0(local0)) {
		int result;
		BEGIN_KMEM
...
int redirecting_sys_rename(const char *oldpath, const char *newpath)
{
	char local0[REDIR_BUFSIZE];
	char local1[REDIR_BUFSIZE];
	if(strncpy_from_user(local0, oldpath, REDIR_BUFSIZE)<0) return -EFAULT;
	if(dredirect0(local0)) {
		int result;	if(strncpy_from_user(local1, newpath, REDIR_BUFSIZE)<0) return -EFAULT; redirect0(local1);
		BEGIN_KMEM
